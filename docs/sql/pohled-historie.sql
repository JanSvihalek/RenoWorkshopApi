...
GO
CREATE VIEW dbo.v_renoworkshop_zakazky_historie
AS
SELECT hlv.reference_subjektu AS c_zakazky,
       ...
       CASE WHEN rada.reference_subjektu LIKE '8%' THEN LTRIM(RTRIM(rada.reference_subjektu)) END AS zakazka_rada,
...
FROM   RAS_HEN.RNC_ostra.lcs.ino_srvszak_hlavicka AS hlv
       LEFT OUTER JOIN
       RAS_HEN.RNC_ostra.lcs.organizace AS org
       ON hlv.organizace = org.cislo_subjektu
       LEFT OUTER JOIN
       RAS_HEN.RNC_ostra.lcs.subjekty AS sub
       ON hlv.zpracovatel = sub.cislo_subjektu
       LEFT OUTER JOIN
       RAS_HEN.RNC_ostra.lcs.attribute_valuation_entry AS val
       ON hlv.stav_real = val.db_value_int
          AND val.cislo_subjektu = 64208
       LEFT OUTER JOIN
       RAS_HEN.RNC_ostra.lcs.ino_vozidlo AS voz
       ON hlv.vozidlo = voz.cislo_subjektu
       LEFT OUTER JOIN
       RAS_HEN.RNC_ostra.lcs.ino_znackamodel AS znm
       ON voz.znackamodel = znm.cislo_subjektu
       LEFT OUTER JOIN
       RAS_HEN.RNC_ostra.lcs.ino_srvszak_zakazka AS rada
       ON hlv.zakazka_hlavni = rada.cislo_subjektu
       LEFT OUTER JOIN
       RAS_HEN.RNC_ostra.lcs.subjekty AS tech
